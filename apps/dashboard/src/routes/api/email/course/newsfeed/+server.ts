...
const sendEmailNotification = async (feedId: string, authorId: string, comment?: string) => {
  ...
  const postLink = `https://${feed.org.siteName}.salera.vn/courses/${feed.courseId}?feedId=${feed.id}`;

  if (comment) {
    const emailData = [
      {
        from: `"${feed.org.name} - ClassroomIO" <notify@mail.salera.vn>`,
